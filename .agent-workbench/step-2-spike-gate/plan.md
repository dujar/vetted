...
- learned: ../step-1-repo-scaffold/findings.md  (scaffold is MERGED at 5e2ff35 — crate header shape, workers build mechanics, live-probed RPCs; this step produces the first builder findings.md, which steps 3, 4, 6 build against)

> **Revised** — step-1 findings reconciliation (2026-09-11): three scaffold facts change how the spike is built.
> 1. Task 2's probe crate: native `stylus_sdk::testing` tests do not compile without the template header — copy it from `contracts/core/hello/src/lib.rs`: `#![cfg_attr(not(any(test, feature="export-abi")), no_main/no_std)]` + `#[macro_use] extern crate alloc;` (storage macros expand `alloc::` paths).
> 2. Task 7's fetch script: the current workers-rs template builds via `worker-build@^0.8` (cargo install + `worker-build --release`, output `build/index.js`) — knowledge workers-rust.md's "no separate build step" is stale; `scan-backend/wrangler.toml` and its live hello worker are the working reference, and wrangler OAuth already has deploy scopes.
> 3. RPC endpoints for 4663/46630 are already live-probed and recorded (`packages/shared/wire.md` chains table, `frontend/src/lib/chains.ts`; 46630 = `rpc.testnet.chain.robinhood.com`, `eth_chainId → 0xb626`) — no RPC discovery work in the spike.
