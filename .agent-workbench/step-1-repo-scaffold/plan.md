...
- Wire contract pinned here (source of truth: journeys.md): verdict enum VERIFIED|IMPOSTOR|UNVERIFIED|REVOKED (spec.md:1, journeys.md:14); guard revert reasons verbatim GUARD_NO_RECORD|GUARD_RECORD_REVOKED|GUARD_PAUSED|GUARD_BLOCKLISTED|GUARD_IMPL_MISMATCH (journeys.md:32); scan request/response incl. power-report rows each carrying an evidence-link field (journeys.md:14, spec.md:30) — scan request is query params `{chainId, addr}` per step-4 plan.md:29 (verify.md loose end 10); registry record is the seven-field shape from step-3 plan.md:28 {status, riskFlags, verifiedAt, impl, registrar, revokedAt, reason} (verify.md loose end 2); chain ids 4663 mainnet / 46630 scratch / 421614 mirror + RPCs from knowledge robinhood-chain.md:14–21, arbitrum-sepolia.md:11–13 — for 46630 fetch the RPC from docs.robinhood.com/chain/connecting and live-probe `eth_chainId == 0xb626` before writing chains.ts (verify.md loose end 3).
- Registry read ABI shipped here (`packages/shared/abi.ts`: human-readable entries + exported selector constants) — signatures fixed from step-3's plan text (verify-verified 2026-09-12): `getRecord(address token)`, `verify(address token, uint256 riskFlags, address impl)`, `revoke(address token, string reason)`, `commit(address tokenIn, address tokenOut, uint256 amountIn, uint256 minOut)`, `execute()`, plus the `GUARD_*` revert-reason constants. Steps 5/8 consume real ABI before any contract exists; `PROBE_SELECTORS` (`paused()`, blocklist probe) ship as clearly-marked placeholder bytes that step 2 replaces with its calibrated ones at merge.
...
## Open questions
- Operator actions, day 1: WalletConnect Cloud projectId; Cloudflare account + API token (wrangler whoami confirms current token lacks workers/pages scopes — deploys fall back to `blocked-auth` entries in deployments/*.json per verify.md loose end 6). None block scaffold code; GitHub push is available (gh authenticated).
- Repo visibility: **PUBLIC — decided 2026-09-12 by the coordinator** (plan's recommendation adopted; verify.md loose end 7).
