...
> 3. **Task 5 handoff is two secrets + the worker's registry-address vars:** step 4's live worker (`vetted-scan-backend.dujar-coding.workers.dev`, version 0ccf8c68) ships with `REGISTRAR_KEY` and `DRIFT_ADMIN_SECRET` both DELIBERATELY UNSET (live-verified 2026-09-20: drift-check degrades with a clean skipped report). Generate/set both — the registrar key per the task below, plus `DRIFT_ADMIN_SECRET` for the admin-gated `POST /registry/drift-check` step 9's upgrade→revocation beat calls — AND set the Worker's `REGISTRY_ADDRESS_4663` wrangler var from `deployments/4663.json` immediately after the deploy (judge round-4 gap 1): the worker reads `REGISTRY_ADDRESS_{4663,46630,421614}` (`scan-backend/src/lib.rs:107–109`, empty in `wrangler.toml:13–15`) and serves `/scan` with `record: null` (canonical-fetch-only degrade) while they're empty — which breaks step-8's live REVOKED spec and leaves step-9's drift-check beat with no records to poll.
