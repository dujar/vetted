...
> 5. **Coordinator decision (2026-09-21, closing judge round-4 gap 1): the guard `erc20()` junk-decode is CARRIED TO JUDGES' Q&A — NOT a sanctioned contracts change.** Task 5's submission package carries a prominent security-note disclosure of it; task 6's Q&A list names it. Worst case (full analysis: `../step-7-contract-hardening/findings.md` "Out of scope, left broken"): a lying token's returndata with byte 31 = 1 decodes as "true" → a phantom active order for the buyer who chose that token → execute reverts whole → funds safe. Product code stays frozen — a freeze-time builder DISCLOSES this, never tightens the decode.

> **Revised** — step-9 findings reconciliation (2026-09-21): step 9 merged unfunded (`../step-9-demo-harness/findings.md` — tasks 1–5 built + rehearsed, task-6 dry runs STAGED):
> 1. **The go-list IS the production deploy path.** If `deployments/4663.json` is still absent when step 10 starts, execute step-9's task-6 go-list FIRST (its findings, "Task-6 go-list" items 1–8: step-7 funded-run runbook → 4663 replicas BEFORE the core deploy's stage F → ONE worker reconfig re-passing ALL non-empty vars + secrets → seed-demo → one live playwright pass → serve → two ≤5:00 dry runs). Task 2's production state check then verifies rather than builds; task 3's smoke runs on its output. If funding has landed and the go-list already ran at step 9, this note is a no-op — audit, don't redo.
> 2. **Product name RESOLVED: "Vetted" is LOCKED** (coordinator, standing delegation; tracker commit fc8b004 on main). Recording and the task-5 package are no longer name-blocked; `frontend/src/lib/brand.ts` is final as-is. Task 5 also uncomments the `demo.gif` img in README once the final take exists (step 9 left the link commented on purpose — links must resolve).
> 3. **P-receipt routing (refines the capture clause in the 2026-09-20 note 2):** the still-missing VERIFIED-shaped live P receipt is chased during step-9's task-6 BETWEEN-TAKES (`live-check.sh` against the production worker, spaced attempts — go-list step 7) and recorded in STEP-9's findings when one lands. Step 10 links it from there as the submission asset; task 3's smoke chases it only if step-9's findings still show it pending at step-10 start.
> 4. **A smoke re-run is not a re-seed:** registry transitions are monotone — a REVOKED record cannot re-verify, and seed-demo exits with the recovery path instead of reverting. A failed task-3 pass recovers via step-9's runbook §6 (re-deploy the replica cast for a fresh proxy), then continues.
...
5. Submission package: final video, README (carrying the PROMINENT security-note disclosure of the guard `erc20()` junk-decode — Revised 2026-09-21 note 5, a required package item, not optional README prose), repo link, live URLs, contract addresses — uploaded targeting **2026-10-03 EOD SGT**, ≥24h ahead of the close (spec.md:69).
6. Post-submit: archive findings, note residual risks for judges' Q&A (fresh-redeployment-reads-IMPOSTOR lag = spec.md:79, twins-on-production mitigation = spec.md:80, and the guard `erc20()` junk-returndata decode — coordinator-carried, disclosed in the submission security note: Revised 2026-09-21 note 5).
